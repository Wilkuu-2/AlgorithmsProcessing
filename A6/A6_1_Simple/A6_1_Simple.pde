...
static final int flowerAmount = 10; 
float timeParameter = millis() * timeFactor;
long nanosElapsed = System.nanoTime();

// -- INIT
void setup() {
  springs = new SpringElement[flowerAmount][flowerHeight];
  size(800, 800, P2D);
...
void draw() {
  ...
  for (int i = 0; i < springs.length; i++) {
    ...
    translate(width/float(flowerAmount)*i, 0); // Horizontal placement
