...
class Landscape {
  ...
  float getLandscapeHeight(float x){
    return noise((x+offX)/rez);
  }
  
  PVector getLandscapePoint(float x){
    return new PVector(x,getLandscapeHeight)
  }
  ...
  void update() {
    ...
    for (int w = 0; w < size.x; w++) { //Iterate over width
      for (int h = floor(size.y-(getLandscapeHeight(w) * size.y)); h < size.y; h++) //Iterate down over height, starting from the noise fuction result
